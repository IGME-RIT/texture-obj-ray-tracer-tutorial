...
{
	vec4 triangleColor = vec4(triangles[i.index].color, 1);

	vec2 uv = GetInterpolatedUV(
		i.point,
		triangles[i.index].pos[0],
		triangles[i.index].pos[1],
		triangles[i.index].pos[2],
		vec2(triangles[i.index].uv[0]),
		vec2(triangles[i.index].uv[1]),
		vec2(triangles[i.index].uv[2])
	);

	return texture(textureTest, uv.xy) * triangleColor;
}
